...
template<typename CI> struct job_position
{
	...
	job_position(machine_settings_t const& mst, CI ctb, CI cte)
		: mst_(mst), ctb_(ctb), cte_(cte), scr_(0)
	{
	}
};

template<typename J, typename CI> auto make_job_list_position(machine_settings_t mst, CI ctb, CI cte) -> std::vector<J>
{
	std::vector<J> vjb;
	...
	w3.Position(position('A', 'A', 'A'));
	do
	{
		mst.pos_ = w3.Position();
		vjb.emplace_back(mst, ctb, cte);
		w3.Step();
		if (w3.Position() == position('A', 'A', 'A'))
			bend = true;
	} while (!bend || --ctl);
	return vjb;
}
	...
	try
	{
		...
		for (auto& j : vjbw)
		{
			...
			do
			{
				...
				auto vjb = make_job_list_position<job_position_t>(j.mst_, j.ctb_, j.cte_);
